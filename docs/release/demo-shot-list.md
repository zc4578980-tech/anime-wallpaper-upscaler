...
Use the checked-in `docs/assets/demo-source-original.png` by default. It is generated by
`scripts/build_original_demo.py`, carries MIT provenance metadata, and uses four different
colored edge markers so the preserve behavior is visible without third-party artwork.
